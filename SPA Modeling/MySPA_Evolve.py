...
# gene classes for chamber volumes


class OrgGene1(FloatGene):

    # gene to refer to the length of chamber
...
class OrgGene2(FloatGene):
    ...
    # gene to refer to the height of chamber
...
class CAEOrganism(Organism):
    ...
    def fitness(self):
        # Best fitness is of the largeset displacement

        inputList = self.getDataSet()
        fit_result = 0.0
        fit_result = -1.0 * performCAE(inputList)

        return fit_result

    def getDataSet(self):

        Lvalue = float(self.genes['a'].value)
        Hvalue = float(self.genes['b'].value)

        Lvalue = round(Lvalue,1)
        Hvalue = round(Hvalue,1)

        SPData = [Lvalue, Hvalue]
        return SPData
...
def performCAE(polist):
    ...
    wall = 2*(body_W-chamber_W)
    cwall = 3*wall/4
    gap = wall/3
    uplayer = (30 - chamber_H)/3
    btlayer = 2*uplayer
    ...
    try:
        ...
        o = session.openOdb(name=jname+'.odb')
...
def main():
    ...
    while i<40:
        ...
        for org in pop.organisms:
            outStr = []
            [outStr.append(round(org.genes[key].value,1)) for key in 'ab']
